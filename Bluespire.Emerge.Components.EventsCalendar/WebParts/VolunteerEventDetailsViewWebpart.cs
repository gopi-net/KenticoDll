...
{
    public class VolunteerEventDetailsViewWebpart : EventsCalendarWebPart
    {
        ...
        {
            string queryName = string.Format(EventsConstants.QUERY_GETVOLUNTEEREVENTDETAILS, SiteContext.CurrentSiteName);
        ...
        public virtual DataSet getVolunteerRegisteredForEvent()
        {
            string queryName = string.Format(EventsConstants.QUERY_GETVOLUNTEEREREGISTERED, SiteContext.CurrentSiteName);
            QueryDataParameters parameters = new QueryDataParameters();
            parameters.Add("@OccurenceID", this.OccurenceID);
            DataSet eventsDS = ConnectionHelper.ExecuteQuery(queryName, parameters, null, null);
            return eventsDS;
