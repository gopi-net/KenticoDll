...
public partial class CMSModules_System_Controls_MacrosGrid : CMSUserControl, IUniPageable
{
    ...
    public void ReloadData()
    {
        ...
        if (!StopProcessing && Visible)
        {
            try
            {
                var macros = GetMacros(startIndex, endIndex, pageSize == -1 ? int.MaxValue : (pageSize * (pagerItems.CurrentPage + 11)), ref mTotalItems);
    ...
    private IEnumerable<MacroExpr> GetMacros(int startIndex, int endIndex, int maxTotalRecords, ref int totalRecords)
    {
        ...
        foreach (var objectType in objectTypes)
        {
            ...
            if ((index >= maxTotalRecords) || !CMSHttpContext.Current.Response.IsClientConnected)
            {
                break;
            }
        }
    ...
    /// </summary>
    /// <param name="expression">Expresssion</param>
    ...
    protected void RenderItem(MacroExpr expression)
    {
        ...
        tr.Transformation = "{% Object.GetFullObjectName(true, true) %}";
