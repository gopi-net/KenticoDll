...
public partial class CMSModules_CMS_EmergeCommon_FormControls_EmailInput : EmergeBaseFormEngineUserControl
{
    #region "Properties"
        ...
        if (validEmails)
        {
            if (FieldInfo != null)
            {
                ...
                if (!string.IsNullOrEmpty(FieldInfo.RegularExpression))
                {
                    ...
                    {
                        ValidationError = FieldInfo.Properties["validationerrormessage"].ToString();
                        return false;
                    }
                }
