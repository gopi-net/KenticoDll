...
public partial class CMSModules_CMS_EmergePortalEngine_UI_PageLayouts_PageLayout_Edit : CMSModalDesignPage
{
    #region "Constants"
    ...
    /// </summary>
    private const string HELP_TOPIC_LINK = "7pFP";
    ...
    private void EnsureDialogMasterPage()
    {
        if (DialogMode)
        {
            ...
            if (!TabMode)
            {
                ...
                PageTitle.TitleInformation = GetTitleInformation();
            }
        }
    }
    ...
    private string GetTitleInformation()
    {
        if (DeviceProfile != null)
        {
            PageTemplateDeviceLayoutInfo deviceLayout = PageTemplateDeviceLayoutInfoProvider.GetTemplateDeviceLayoutInfo(PageTemplateId, DeviceProfile.ProfileID);
            bool layoutIsMapped = (deviceLayout != null);

            // Check layout auto mapping
            if (!layoutIsMapped && (PageTemplateId > 0))
            {
                layoutIsMapped |= (DeviceProfileLayoutInfoProvider.GetTargetLayoutInfo(DeviceProfile.ProfileID, PageTemplate.LayoutID) != null);
            }
