...
public partial class CMSModules_CMS_EmergePortalEngine_Controls_WebParts_WebPartTree : CMSAdminControl
{
    #region "Variables"
    ...
    private bool mShowWidgetOnlyWebparts;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!ShowWidgetOnlyWebparts)
        {
            ...
            where = SqlHelper.AddWhereCondition(where, "ObjectPath = '" + SqlHelper.GetSafeQueryString(rootPath, false) + "' OR (SELECT Count(*) FROM View_CMS_WebPartCategoryWebpart_Joined AS X WHERE X.ObjectType = N'webpart' AND (X.WebPartType IS NULL OR X.WebPartType != " + Convert.ToInt32(WebPartTypeEnum.WidgetOnly) + ") AND X.ObjectPath LIKE View_CMS_WebPartCategoryWebpart_Joined.ObjectPath + '/%') > 0");
        }

        // Add custom where condition
        if (!string.IsNullOrEmpty(RootPath))
        {
            where = SqlHelper.AddWhereCondition(where, "ObjectPath = '" + SqlHelper.GetSafeQueryString(RootPath, false) + "' OR ObjectPath LIKE '" + SqlHelper.GetSafeQueryString(RootPath, false) + "/%'");
