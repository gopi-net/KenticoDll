...
public partial class CMSModules_CMS_EmergePortalEngine_Controls_WebParts_WebPartFlatSelector : CMSAdminControl
{
    #region "Variables"
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        {
            flatElem.WhereCondition = SqlHelper.AddWhereCondition(flatElem.WhereCondition, "WebPartCategoryID = " + SelectedCategory.CategoryID + " OR WebPartCategoryID IN (SELECT CategoryID FROM CMS_WebPartCategory WHERE CategoryPath LIKE N'" + SqlHelper.GetSafeQueryString(SelectedCategory.CategoryPath, false).TrimEnd('/') + "/%')");
        }

        // Recently used
        if (TreeSelectedItem.ToLowerCSafe() == "recentlyused")
        {
            flatElem.WhereCondition = SqlHelper.AddWhereCondition(flatElem.WhereCondition, SqlHelper.GetWhereCondition("WebPartName", MembershipContext.AuthenticatedUser.UserSettings.UserUsedWebParts.Split(';')));
        }
