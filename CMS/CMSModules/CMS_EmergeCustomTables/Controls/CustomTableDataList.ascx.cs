...
public partial class CMSModules_CMS_EmergeCustomTables_CustomTables_Controls_CustomTableDataList : CMSUserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        gridData.OnAfterRetrieveData += gridData_OnAfterRetrieveData;
    ...
    DataSet gridData_OnAfterRetrieveData(DataSet ds)
    {
        if (!DataHelper.DataSourceIsEmpty(ds))
        {
            if (CustomTableClassInfo != null)
            {
                ...
                List<string> encryptedColumns = null;
                ...
                else
                {
                    ...
                    encryptedColumns = new List<string>();
                ...
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    foreach (string column in encryptedColumns)
                    {
                        ...
                        string newValue = EncryptionHelper.DecryptData(value);
                        row[column] = newValue;
                    }
                }
            }
    ...
    private bool CheckPermissions(string permissionName)
    {
        ...
        if (!MembershipContext.AuthenticatedUser.IsAuthorizedPerResource("cms.customtables", permissionName) &&
            !MembershipContext.AuthenticatedUser.IsAuthorizedPerClassName(CustomTableClassInfo.ClassName, permissionName))
        {
