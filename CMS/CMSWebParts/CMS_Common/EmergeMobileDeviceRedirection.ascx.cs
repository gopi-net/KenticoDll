...
public partial class CMSWebParts_CMS_Common_EmergeMobileDeviceRedirection : CMSAbstractWebPart
{
    #region "Constants"
    ...
    protected void SetupControl()
    {
        if (!StopProcessing)
        {
            ...
            if ((AlwaysRedirect || String.IsNullOrEmpty(redirected)) && (PortalContext.ViewMode == ViewModeEnum.LiveSite))
            {
                ...
                if (!string.IsNullOrEmpty(redirectUrl) &&  null ==  CookieHelper.GetValue(Constants.COOKIE_NAME_MOBILEFULLSITELINK))
                {
                    CookieHelper.Remove(Constants.COOKIE_NAME_MOBILEFULLSITELINK);
                    string newURL = URLHelper.ResolveUrl(redirectUrl);
                    newURL = RequestContext.CurrentURL.Replace(SystemContext.ApplicationPath, newURL);
                    // If current URL is same as set, no redirection is done
                    if ((RequestContext.CurrentURL != newURL) && (URLHelper.GetAbsoluteUrl(RequestContext.CurrentURL) != newURL))
                    {
