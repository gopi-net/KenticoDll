...
{
    public class AdHocPageTemplateProcessor : ObjectProcessor
    {
        ...
        public AdHocPageTemplateProcessor()
        {
            if (String.IsNullOrEmpty(FileName))
            {
                FileName = PackageSettingsHelper.GetPackageFile(Constants.ADHOCPAGETEMPLATESFILEPATH);
            }
        }

        public override void ProcessXML()
        {
            LogProgress(string.Format("Processing Ad-Hoc Page Templates xml - {0}.", FileName));
            string filePath = PackageHelper.TargetFolder + FileName;
            XDocument xmlDoc = XDocument.Load(filePath);
            processPageTemplateXML(xmlDoc);

            xmlDoc.Save(filePath);
            LogProgress(string.Format("Ad-Hoc Page Templates xml - {0} processed successfully.", FileName));
        }
        ...
        private void processPageTemplateXML(XDocument xmlDoc)
        {
            ...
            {
                LogProgress(string.Format("Ad-Hoc Page Template - {0}.", item.Element(PAGETEMPLATEDISPLAYNAME).Value));
